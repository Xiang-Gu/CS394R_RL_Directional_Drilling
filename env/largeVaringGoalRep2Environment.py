...
class LargeVaringGoalRep2Env(gym.Env):
    
    simstates = np.zeros((1,4)) #Continuous state variables
    
    def __init__(self):
        super(LargeVaringGoalRep2Env, self).__init__()
        # Define action and observation space
        # They must be gym.spaces objects
        self.action_space = spaces.Discrete(21)
        
        #Drilling state bounds:
        self.stateLow = np.array([0., 0., -10., -25., 450., 400.])
        self.stateHigh = np.array([1000, 500, 100, 25, 550., 500.])
        
        self.observation_space = spaces.Box(low=self.stateLow, high=self.stateHigh, dtype=np.float64)

        # large varing goal area.
        while True:
            self.X = np.random.uniform(low=450, high=550) #Size in feet of position grid x perimeter, 1000
            self.Y = np.random.uniform(low=400, high=500) #Size in feet of position grid y perimeter, 500
            if not (475 < self.X < 525 and 425 < self.Y < 475):
                break
