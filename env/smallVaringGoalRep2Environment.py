...
class SmallVaringGoalRep2Env(gym.Env):
    
    simstates = np.zeros((1,4)) #Continuous state variables
    
    def __init__(self):
        super(SmallVaringGoalRep2Env, self).__init__()
        # Define action and observation space
        # They must be gym.spaces objects
        self.action_space = spaces.Discrete(21)
        
        #Drilling state bounds:
        self.stateLow = np.array([0., 0., -10., -25., 475., 425.])
        self.stateHigh = np.array([1000, 500, 100, 25, 525., 475.])
        
        self.observation_space = spaces.Box(low=self.stateLow, high=self.stateHigh, dtype=np.float64)

        self.X = np.random.uniform(low=475, high=525) #Size in feet of position grid x perimeter, 1000
        self.Y = np.random.uniform(low=425, high=475) #Size in feet of position grid y perimeter, 500
...
#            ku=10
        ...
        y_check = ((y<self.stateLow[1]) | (y>self.Y))
...
#            r=-250
        ...
        if out_of_bounds:
            self.e_pos = -np.sqrt((x[-1]-self.X)**2+(y[-1]-self.Y)**2)
            self.r_pos = self.e_pos
            self.r_ang = -2*(abs(90-theta[-1]))
            self.r_cur = -np.max(abs(simstates[:,3]*100))
            r+= self.r_pos + self.r_ang + self.r_cur
            return np.append(self.state, [self.X, self.Y]), r, True, {}
        else:
            r+=0
            return np.append(self.state, [self.X, self.Y]), r, False, {}
...
#        simstates = so.reshape(1,-1)
        ...
        # state representation 2 = (x, y, theta, thetadot, goal_x, goal_y)
        return np.append(self.state, [self.X, self.Y])
