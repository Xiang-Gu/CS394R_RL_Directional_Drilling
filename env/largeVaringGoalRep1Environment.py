...
class LargeVaringGoalRep1Env(gym.Env):
    
    simstates = np.zeros((1,4)) #Continuous state variables
    
    def __init__(self):
        super(LargeVaringGoalRep1Env, self).__init__()
...
#            r=-250
        ...
        # newState = (delta_x, delta_y, delta_theta, thetadot)
        self.newState = np.append([self.X, self.Y, degrees(atan((self.Y-yo)/(self.X-xo)))]- self.state[:-1], self.state[-1])
        ...
        if out_of_bounds:
            ...
            return self.newState, r, True, {}
        else:
            r+=0
            return self.newState, r, False, {}
