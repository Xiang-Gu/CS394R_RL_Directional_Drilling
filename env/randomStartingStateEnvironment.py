...
class RandomStartingStateEnv(gym.Env):
    
    simstates = np.zeros((1,4)) #Continuous state variables
    
    def __init__(self):
        super(RandomStartingStateEnv, self).__init__()
...
#        simstates = np.zeros((1,4))
        ...
        yo = np.random.uniform(low=70, high=110) # 90+-20
        xo = np.random.uniform(low=0, high=20)   # 0
