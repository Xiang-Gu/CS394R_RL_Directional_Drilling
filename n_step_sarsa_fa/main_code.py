...
from tc2 import ValueFunctionWithTile
from episodic_semi_gradient_sarsa_py import episodic_semi_gradient_sarsa
...
class EpsGreedy(Policy):
    ...
    def action(self, V, state): #Done
        
        if np.random.rand(1) < self.eps:
#            print('Suboptimal action taken')
            action = np.random.choice(np.arange(0,21,1))
        else:
            #Break ties randomly
            aval = np.zeros(21)
            for action in range(21):
                #state should be action numpy array of state dimensions
                #estimated action value
                aval[action] = V.__call__(np.append(state,action))
            action = np.random.choice(np.flatnonzero(aval == np.max(aval)))
#            print('Number of optimal actions: '+str(np.shape(np.flatnonzero(aval == np.max(aval)))))
#            print(np.flatnonzero(aval == np.max(aval)))
            #Break ties by choosing first max in array
#            action = np.argmax(aval)
        return action
...
if __name__ == "__main__":
    for seed in range(5):
        ...
        random_policy = RandomPolicy(21) #argument is nA (number of actions)
        num_tilings = 16
        initw = 0

        num_tiles = np.append(drill_world.sD/drill_world.tile_width,21)
        V = ValueFunctionWithTile(num_tilings,num_tiles,initw) #tilings, #tiles, initial w
        
        #Our online behavior policy for online GPI 
        episodic_policy = EpsGreedy(V,.05) #eps, alpha 


        print("Initiating Learning with semi-gradient n-step sarsa. seed = " + str(seed))
        ...
        while True:
            ...
            while done is False:
                action = episodic_policy.action(V, state) # Online policy (epsilon-greedy)
            ...
            #n_steps is set to action large value of 100 to implement monte carlo updates
            episodic_semi_gradient_sarsa(drill_world.spec, traj, n=20, alpha=.006, V=V)
       

        np.savetxt('FA_on_policy_n_step_sarsa_seed' + str(seed) + '.npy', maxEpRets)
